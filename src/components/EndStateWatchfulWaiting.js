...
import { Eye, RotateCcw, Copy, FileText, ArrowLeft } from 'lucide-react'
import SaveVisitSummaryButton from './SaveVisitSummaryButton.js'

export default function EndStateWatchfulWaiting({ onReset, pathSummary, onBack, canGoBack , patientId, patientData, endState }) {
  ...
  return React.createElement('div', { className: 'bg-white rounded-xl border border-slate-100 overflow-hidden', style: { boxShadow: '0 8px 40px -8px rgba(245,158,11,0.2)' } },
    ...
    React.createElement('div', {
      ...
      React.createElement('h2', { className: 'text-xl sm:text-2xl font-bold text-white mb-1' }, 'Discuss / Offer Watchful Waiting'),
